The Casino 4 Client Development Harness

I. Description

The purpose of the Casino 4 Client Development Harness is to allow client developers to perform high-frequency
...
Both Standalone and Live modes require that client Casino 4 Framework files be downloaded from a running instance of the
...
placed in the root of the harness/webapp/casino4/standalone directory. At a minimum, the following standalone XML files are
...
The files above are pre-installed in the webapp/casino5/standalone directory. They will require some minor editing to work
...
with game-specific settings and files, it is recommended that the harness remain in the pre-installed location: the trunk/client/casino4
...
files to be stored. The following is a list of the files and directories under "harness".
    ...
    - webapp/casino4/standalone - Stores the standalone response XML files used by the standalone harness.
...
The Casino 4 Client Development Harness uses four configuration files to store game and environment-specific settings. All
...
o standalone.properties
  ...
  properties (which are documented within as well).
    ...
    - base, data, js, egiURL, and egiExtGameId - Special parameters that are only needed for EGI Game Development. You
          can safely ignore these (leave them unset).
...
o webserver.properties
  ...
  This file stores the values that define the properties used by the Tomcat webserver instance. Be sure to use forward slashes in all paths.
    ...
    - tomcat.start.port - Port to use when running Tomcat. Default is 7777. This value does not need to be changed but can be modified if this port
                         conflicts with another process running on the machine.
    - tomcat.stop.port - Port to use when stopping Tomcat. Default is 7778. This value does not need to be changed but can be modified if this port
                        conflicts with another process running on the machine.
...
has already been executed).
    ...
    - Standalone: http://localhost:7777/standalone/harness.html
    - Live: http://localhost:7777/live/harness.html
