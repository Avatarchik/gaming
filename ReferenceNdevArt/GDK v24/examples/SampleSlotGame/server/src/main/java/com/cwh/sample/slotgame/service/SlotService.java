...
public class SlotService {
    ...
    public void resetGameService(GameRequest gameRequest) {
        ...
        if(gameRequest instanceof SpinGameRequest) {
            ...
            this.gameState.setCreditPerLine(spinGameRequest.getCreditsPerLine());
    ...
    public List<Transaction> generateInstructions(GameRequest gameRequest) {
        ...
        if(gameRequest instanceof SpinGameRequest) {
            ...
            Integer creditWager = SpinEvaluator.PAYLINES.length * spinGameRequest.getCreditsPerLine();
            this.gameState.setCreditWager(creditWager);
    ...
    public GameResponse generateGameResponse(GameRequest gameRequest) {
        ...
        if(gameRequest instanceof InitGameRequest) {
            ...
            ((InitGameResponse)gameResponse).setNumberSevensFound(this.persistentState.getNumBonusSymbols());
        } else if(gameRequest instanceof SpinGameRequest || gameRequest instanceof FreeGameRequest) {
            gameResponse = new SpinGameResponse();
            // Generate the Response to the Client
            ((SpinGameResponse)gameResponse).setReels(this.gameState.getReels());
            ((SpinGameResponse)gameResponse).setSpinResults(this.gameState.getSpinResults());
            ((SpinGameResponse)gameResponse).setNumberSevensFound(this.persistentState.getNumBonusSymbols());
            ...
            runningResult.setCreditsWagered(this.gameState.getCreditWager());
        ...
        if(this.gameState.getPlayState().equals(GameState.PLAY_STATE.FINISHED)) {
            ...
            summaryResult.setCreditsWagered(GameRounding.shiftForCredits(this.gameState.getCreditValue().multiply(new BigDecimal(this.gameState.getCreditWager()))));
            summaryResult.setCreditsWon(GameRounding.shiftForCredits(this.gameState.getCreditValue().multiply(new BigDecimal(this.gameState.getCreditWonTotal()))));
     ...
     */
    public void processReelSpin(RandomPools randomPools) {
