...
public class ReplayGameResponse {
	...
	private String addReferences() {
		return "<tr style='display: none'> <td><input type='hidden' name='css_path' value='/casino/remote/MonarchSun/replay/styles/monarchsun.css' /> <script language='javascript' src='/casino/remote/MonarchSun/replay/javascript/jquery-1.7.2.min.js' type='text/javascript'> placeholder </script> <script language='javascript' src='/casino/remote/MonarchSun/replay/javascript/utils.js' type='text/javascript'> placeholder </script> <script language='javascript' src='/casino/remote/MonarchSun/replay/javascript/monarchsun.js' type='text/javascript'> placeholder </script></td></tr>";
	}
	...
	private String addHeader(Node replay) {
		...
		+ "<p class='value'>Monarch Sun</p>" + "</div>" + "<div>"
	...
	private String addGames(Node replay) {
		...
		for (int i = 0; i < replayGames.size(); i++) {
			...
			for (int j = 0; j < reels.size(); j++) {
				...
				for (String iconID : iconsS.split(",")) {
					returnString += "<img class='row" + (k + 1) + " col"
							...
							+ "src='/casino/remote/MonarchSun/replay/images/"
			...
			/*for (int m = 0; m < MonarchSunConfig.NUMBER_OF_LINES / 2; m++) {
				returnString += "<div id='payline_" + (m + 1)
						+ "' class='payline_button'" + "style='top: "
						+ (m * 20) + "px; left: 0px'>" + (m + 1) + "</div>";
			}
			for (int n = 0; n < MonarchSunConfig.NUMBER_OF_LINES / 2; n++) {
				returnString += "<div id='payline_"
						+ (n + MonarchSunConfig.NUMBER_OF_LINES / 2 + 1)
						+ "' class='payline_button'" + "style='top: "
						+ (n * 20) + "px; right: 0px'>"
						+ (n + MonarchSunConfig.NUMBER_OF_LINES / 2 + 1)
						+ "</div>";
				...
				for (int p = 0; p < winningLines.size(); p++) {
					...
					if (!lineId.equals("0")) {
						...
						for(int q = 0; q < 5; q++) {
							...
							for(int r = 0; r < postionsStrSep.length && !added; r++) {
								...
								if(col == q+1) {
									...
									imgsStr += " <img id='main_winWay_1_r"+row+"c"+col+"' style='float:left;width:50px;height:inherit;margin:0px;padding:0px' src='/casino/remote/MonarchSun/replay/images/" + reelDisplay[row-1][col-1] + "_Symbol.png' alt='"+ reelDisplay[row-1][col-1] + "_Symbol.png' />";
									added = true;
								}
							}
						}
						...
						scatterWinningsString = "<div class='winning_lines'>"
								...
								+ " <img id='main_winWay_1_r2' style='float:left;width:50px;height:inherit;margin:0px;padding:0px' src='/casino/remote/MonarchSun/replay/images/11_Symbol.png' alt='11_Symbol.png' />"
								+ " <img id='main_winWay_1_r3' style='float:left;width:50px;height:inherit;margin:0px;padding:0px' src='/casino/remote/MonarchSun/replay/images/11_Symbol.png' alt='11_Symbol.png' />"
								+ " <img id='main_winWay_1_r4' style='float:left;width:50px;height:inherit;margin:0px;padding:0px' src='/casino/remote/MonarchSun/replay/images/11_Symbol.png' alt='11_Symbol.png' />"
								+ "</div>"
								+ "<p>"
								+ "Won "+ numInReel[0]*numInReel[1]*numInReel[2]*MonarchSunConfig.FREE_SPINS_AWARDED + " Free Games</p>"
								+ "<input type='hidden' name='positions' "
								+ "value='"
								+ positionsStr + "' />"
	...
	private String addResultSummary(Node replay) {
		...
		+ (int) (creditWager * MonarchSunConfig.COST_TO_COVER)
		+ "</p>"
		+ "</div>"
		+ "<div>"
		...
		* creditValue
		* MonarchSunConfig.COST_TO_COVER)
		+ "</p>"
		+ "</div>"
		+ "<div>"
		...
		* creditValue)
		...
		+ "<p class='value'>"
		+ numberFormat.format(((totalWon * creditValue) - (creditWager * creditValue * MonarchSunConfig.COST_TO_COVER)))
				+ "</p>"
				+ "</div>"
				+ "</div></td></tr>";
		} else {
