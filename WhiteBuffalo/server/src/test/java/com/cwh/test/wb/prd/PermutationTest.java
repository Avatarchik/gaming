...
import com.cwh.slotstoolbox2.DefaultPatternMatcher;
import com.cwh.slotstoolbox2.PatternMatcher;
import com.cwh.slotstoolbox2.PatternPayout;
...
import com.cwh.wb.service.WhiteBuffaloBonusEvaluator;
import com.cwh.wb.service.WhiteBuffaloEvaluator;
import com.cwh.wb.service.WhiteBuffaloREEvaluator;
...
public class PermutationTest {
	...
	public void RunPermutationTest() {
		...
		try {
			...
			int reel5Size = WhiteBuffaloReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(levelInt)).get(5).size();
...
class PermutationThread extends Thread {
	String[] symbols = new String[46];
	private WhiteBuffaloPayTableConfig mainPayConfig = new WhiteBuffaloPayTableConfig();
	...
	public void run() {
		...
		long reel1Size = WhiteBuffaloReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(1).size();
		long reel2Size = WhiteBuffaloReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(2).size();
		long reel3Size = WhiteBuffaloReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(3).size();
		long reel4Size = WhiteBuffaloReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(4).size();
				...
				for(int c = 0; c < reel3Size; c++) {
					for(int d = 0; d < reel4Size; d++) {
						for(int e = start; e < finish; e++) {
							...
							symbolsPattern.add(subListCircular(WhiteBuffaloReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(1),a, a+4));
							symbolsPattern.add(subListCircular(WhiteBuffaloReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(2),b, b+4));
							symbolsPattern.add(subListCircular(WhiteBuffaloReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(3),c, c+4));
							symbolsPattern.add(subListCircular(WhiteBuffaloReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(4),d, d+4));
							symbolsPattern.add(subListCircular(WhiteBuffaloReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(5),e, e+4));

							List<PowerXStreamPaylineResult> paylineResults = WhiteBuffaloEvaluator.EvaluateL2R(mainPayConfig, symbolsPattern, WhiteBuffaloConfig.COST_TO_COVER, false, null);
							paylineResults.addAll(WhiteBuffaloEvaluator.EvaluateR2L(mainPayConfig, symbolsPattern, WhiteBuffaloConfig.COST_TO_COVER, null));

							//Evaluate scatter wins
							PowerXStreamPaylineResult scatterWin = WhiteBuffaloBonusEvaluator.ScatterEvaluate(symbolsPattern, WhiteBuffaloConfig.FREE_SPINS_AWARDED, WhiteBuffaloConfig.SYMBOL_BONUS, null);
