...
public class PermutationFreeSpinTest {
	...
	public void RunPermutationTest() {
		...
		try {
			...
			int index = 0;
			for(WinPattern wp : WhiteBuffaloConfig.PAY_TABLE.getWinPatterns()) {
				symbols[index] = wp.symbol();
				counts[index] = wp.count();
				index++;
			}
			symbols[33] = WhiteBuffaloConfig.SYMBOL_SCATTER;
			counts[33] = 3;

			int cores = Runtime.getRuntime().availableProcessors();
			int reel5Size = WhiteBuffaloFreeSpinsReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(levelInt)).get(5).size();
...
class PermutationFreeSpinThread extends Thread {
	...
	public void run() {
		...
		PatternMatcher matcher = DefaultPatternMatcher.createDefaultMatcher(WhiteBuffaloConfig.PAY_TABLE, WhiteBuffaloConfig.WILD_SYMBOL);
		long reel1Size = WhiteBuffaloFreeSpinsReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(1).size();
		long reel2Size = WhiteBuffaloFreeSpinsReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(2).size();
		long reel3Size = WhiteBuffaloFreeSpinsReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(3).size();
		long reel4Size = WhiteBuffaloFreeSpinsReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(4).size();
				...
				for(int c = 0; c < reel3Size; c++) {
					for(int d = 0; d < reel4Size; d++) {
						for(int e = start; e < finish; e++) {
							...
							symbolsPattern.add(WhiteBuffaloFreeSpinsReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(1).get(a));
							symbolsPattern.add(WhiteBuffaloFreeSpinsReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(2).get(b));
							symbolsPattern.add(WhiteBuffaloFreeSpinsReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(3).get(c));
							symbolsPattern.add(WhiteBuffaloFreeSpinsReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(4).get(d));
							symbolsPattern.add(WhiteBuffaloFreeSpinsReelsConfig.getReels().get(WhiteBuffaloRTP.rtp(rtpLevel)).get(5).get(e));
