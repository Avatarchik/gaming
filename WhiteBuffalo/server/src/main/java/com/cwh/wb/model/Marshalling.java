...
public class Marshalling
{
	public static Node PaylineResultMarshal (PowerXStreamPaylineResult paylineResult)
	{
		...
		ePayline.addAttribute(new Attribute("icon", WhiteBuffaloConfig.SYMBOL_MAP.get(paylineResult.symbol())));
	...
	public static PowerXStreamPaylineResult PaylineResultUnMarshal (Node ePaylineResults)
	{
		...
		for(int i = 0; i < WhiteBuffaloConfig.NUM_REELS; i++) {
		...
		for (String pos : listSymbolsToMatch)
		{
			...
			posList.get(reelId-1).add(WhiteBuffaloConfig.REVERSE_MAP.get(Integer.valueOf(posStrings.get(1))-1));
			posListInteger.get(reelId-1).add(Integer.valueOf(posStrings.get(1))-1);
		}
		...
		PowerXStreamPaylineResult rv = PowerXStreamPaylineResult.newWin(newPayline,
				creditsWon,
				WhiteBuffaloConfig.REVERSE_MAP.get(iconWon),
				posListInteger.size(),
				posList);
		return rv;
	}
	...
	public static void ReelsMarshal(Reels eReels, Element eReelsNode)
	{
		...
		for (int reelCount = 0; reelCount < reelDisplay.size(); reelCount++)
		{
			...
			if(reelCount == 0 || reelCount == WhiteBuffaloConfig.NUM_REELS-1) {
				stopIndex--;
			}
			for (int symbolCount = 0; symbolCount < stopIndex; symbolCount++)
			{
				reel.appendChild(WhiteBuffaloConfig.SYMBOL_MAP.get(eReels.getReelsDisplay().get(reelCount).get(symbolCount)));
	...
	public static void ReelsMarshal(List<List<String>> reelDisplay, List<Integer> stopPositions, Element eReelsNode)
	{
		for (int reelCount = 0; reelCount < reelDisplay.size(); reelCount++)
		{
			...
			for (int symbolCount = 0; symbolCount < stopIndex; symbolCount++)
			{
				reel.appendChild(WhiteBuffaloConfig.SYMBOL_MAP.get(reelDisplay.get(reelCount).get(symbolCount)));
				if (symbolCount + 1 < stopIndex)
				{
					reel.appendChild(",");
				}
	...
	public static List<String> ReReplacementUnMarshal(Nodes nReReplacement) {
		...
		{
			reReplacement.add(WhiteBuffaloConfig.REVERSE_MAP.get(stopsStringList.get(i)));
		}
	...
	public static void ReReplacementMarshal(List<String> rePattern, Element eReelsNode) {
		...
		{
			rePatternE.appendChild(WhiteBuffaloConfig.SYMBOL_MAP.get(rePattern.get(i)));
	...
	public static void ReelsMapMarshal(Map<Object, Map<Integer, List<String>>> map, Element eReels) {
		for (int level = 1; level < WhiteBuffaloRTP.values().length+1; level++) {
			Element rtp = new Element("rtp");
			rtp.addAttribute(new Attribute("level",  String.valueOf(level)));

			for (int reelCount = 0; reelCount < WhiteBuffaloConfig.NUM_REELS; reelCount++)
			{
				Element reel = new Element("reel");
				reel.addAttribute(new Attribute("id",  String.valueOf(reelCount + 1)));
				List<String> reelSymbols = map.get(WhiteBuffaloRTP.rtp(level)).get(reelCount + 1);
				for (int symbolCount = 0; symbolCount < reelSymbols.size(); symbolCount++)
				{
					reel.appendChild(WhiteBuffaloConfig.SYMBOL_MAP.get(reelSymbols.get(symbolCount)));
	...
	public static void ReelsMapMarshal(Map<Object, Map<Integer, List<String>>> map, Element eReels, int rtpLevel) {
		for (int reelCount = 0; reelCount < WhiteBuffaloConfig.NUM_REELS; reelCount++)
		{
			Element reel = new Element("reel");
			reel.addAttribute(new Attribute("id",  String.valueOf(reelCount + 1)));
			List<String> reelSymbols = map.get(WhiteBuffaloRTP.rtp(rtpLevel)).get(reelCount + 1);
			for (int symbolCount = 0; symbolCount < reelSymbols.size(); symbolCount++)
			{
				reel.appendChild(WhiteBuffaloConfig.SYMBOL_MAP.get(reelSymbols.get(symbolCount)));
				if (symbolCount + 1 < reelSymbols.size())
	...
	public static void InitStopsMarshal(Element eInitStops, int rtpLevel) {
		...
		for (int i = 0; i < WhiteBuffaloConfig.NUM_REELS; i++)
		{
			eInitStops.appendChild(WhiteBuffaloReelsConfig.getInitStops(WhiteBuffaloRTP.rtp(rtpLevel)).get(randomNumber).get(i).toString());
			if (i + 1 < WhiteBuffaloConfig.NUM_REELS)
			{
