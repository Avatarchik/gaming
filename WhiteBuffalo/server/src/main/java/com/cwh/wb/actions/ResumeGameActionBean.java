...
public class ResumeGameActionBean extends AbstractGameRequestActionBean {
    ...
    public Resolution processResumeRequest() {
        try {
            ...
            runningResult.setCreditsWagered(gameState.getCreditWager()*WhiteBuffaloConfig.COST_TO_COVER);
            ...
            if(gameState.getPlayState().equals(GameState.PLAY_STATE.FINISHED)) {
                ...
                summaryResult.setWagered((gameState.getCreditValue().multiply(new BigDecimal(gameState.getCreditWager()))).multiply(new BigDecimal(WhiteBuffaloConfig.COST_TO_COVER)));
